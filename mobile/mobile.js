...
const MobileApp = {
    ...
    init: function() {
        ...
        // 1. Core Logic Activation (V36: Desktop Engines)
        if (window.RasMirqabGlobe) RasMirqabGlobe.init();
        
        // V36: Use Desktop Core News Engine directly
        if (window.BreakingNewsWidget) {
            // Override renderer BEFORE init to catch first load
    ...
    initBreakingNews: function() {
        console.log("📡 V36: Desktop Clone Mode Active.");
        // We've already moved the init() and override to MobileApp.init()
        // for maximum reliability during early boot.
    },
