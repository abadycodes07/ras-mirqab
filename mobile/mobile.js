...
const MobileApp = {
    ...
    init: function() {
        console.log('--- 🚀 RAS MIRQAB MOBILE V34: DEFINITIVE REFORM ---');
        this.version = 'v34';
        if (window.RasMirqabGlobe) {
            RasMirqabGlobe.init();
            this.initLayersModal();
        }
        this.initBreakingNews(); // V34: Re-linked to Desktop Core
        this.initTVCarousel();
    ...
    // ═══ V34: DEFINITIVE NEWS ENGINE (RE-LINKED) ═══
    initBreakingNews: function() {
        console.log("📡 V34: Linking to Core News Engine...");
        if (window.BreakingNewsWidget) {
            // Override the renderer for mobile
            window.BreakingNewsWidget.renderItems = (container, items) => {
                this.renderMobileNews(items);
            };
            window.BreakingNewsWidget.init();
        } else {
            console.error("❌ BreakingNewsWidget not found, pulsarian mode");
    ...
    fetchMobileNews: async function() {
        try {
            const response = await fetch('../news-cache.json?t=' + Date.now());
            if (!response.ok) throw new Error('Fetch failed');
            const data = await response.json();
            if (data && data.items) {
                localStorage.setItem('rasmirqab_mobile_news_v34', JSON.stringify(data.items));
                this.renderMobileNews(data.items);
            }
        } catch (e) {
            console.error('--- V34: News Fetch Error:', e);
            const cached = localStorage.getItem('rasmirqab_mobile_news_v34');
