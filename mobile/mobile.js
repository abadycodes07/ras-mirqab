...
const MobileApp = {
    ...
    init: function() {
        ...
        this.initMobileNews(); // V32: Standalone Engine
        this.initTVCarousel();
    ...
    // ═══ V32: STANDALONE NEWS ENGINE ═══
    initMobileNews: function() {
        const list = document.getElementById('news-list');
        if (!list) return;

        // 1. Initial Load from Local Cache
        const cached = localStorage.getItem('rasmirqab_mobile_news_cache');
        if (cached) {
            try {
                this.renderMobileNews(JSON.parse(cached));
            } catch(e) {}
        }

        // 2. Fetch Fresh Data (Directly from Server Cache)
    ...
    fetchMobileNews: async function() {
        try {
            // Fetch directly from the news.json generated by the server/desktop
            const response = await fetch('../news.json?t=' + Date.now());
            if (!response.ok) throw new Error('Fetch failed');
            const data = await response.json();
            
            if (data && data.length > 0) {
                localStorage.setItem('rasmirqab_mobile_news_cache', JSON.stringify(data));
                this.renderMobileNews(data);
            }
        } catch (e) {
            console.error('--- V32: News Fetch Error:', e);
            const list = document.getElementById('news-list');
            if (list && list.innerHTML.trim() === '') this.showNewsSkeleton(list);
        }
    },
    ...
    renderMobileNews: function(items) {
        ...
        displayItems.forEach(item => {
            const handle = (item.sourceHandle || item.source || '').toLowerCase();
